...
export default function RpgLogged() {
  const navigate = useNavigate();
    ...
    useEffect(() => {
        ...
        (async() => {
            try {
                const games = await axios.get(process.env.REACT_APP_BACKEND_URL+"/api/jatek/kategoria/RPG");
                setGame(games.data);
  ...
  return (
    ...
    <div className="p-5  m-auto text-center content bg-ivory">
      {isFetchPending ? (
        ...
        <div>
          <h2 className='jateknevszin'>RPG játékaink:</h2>
          {game.map((game) => (
